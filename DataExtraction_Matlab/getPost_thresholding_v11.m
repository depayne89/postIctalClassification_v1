...
function [] =  getPost_thresholding_v11(iPt)
%% Options and parameters

% Parameters
sz_to_use = 40;         % number os seizures to use in this analysis (takes the first found)
...
%         ix = T_P_data(post_start:end)>(av_av);
        ix = find(T_P_data(post_start:end)>averages(n)+stds(n)/2, 1);        % Find first index past threhsold
