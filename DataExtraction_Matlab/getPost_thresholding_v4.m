...
function [] =  getPost_thresholding_v4(iPt)
...
for n = 1:N
    ...
    search_time = 40;                                    % min, length of post-ictal time to look at
...
% Based on simple eye-balling a decent threshold may lie between 1200 and
% 1600 (at least for patient 11). This assumes the x20 performed for
% visualization. Look again tho
    ...
    threshold = [4000 3500 4000 2000 10000 800 10000 2500 8000 4000 3000 10000 3000 2000 3000];                            % arbitrary from eyballing data                                      % arbitrary from eyballing data
    %ix = find(T_E_data(window_size:end)>threshold, 1)      % Find first point past threhsold
...
ix = find(post_lengths~=0);
post_lengths = post_lengths(ix);
sz_lengths=sz_lengths(ix);


%% Plot Figures

[R,P] = corrcoef(post_lengths,sz_lengths)

sz_shown = size(post_lengths,1);
...
figure
scatter(sz_lengths, post_lengths);
lsline;
title(['Pt - ' num2str(iPt)])
